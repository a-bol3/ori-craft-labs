...
export function LocalizedHome({ locale }: { locale: Locale }) {
  ...
  const cards = [
    [copy.services, copy.servicesText, "Wellness"],
    [copy.offers, copy.offersText, "Culture"],
    [copy.about, copy.aboutText, "Community"],
  ];
    ...
    <main className="min-h-screen bg-brand text-white selection:bg-cta selection:text-brand">
      <section className="relative flex min-h-[86vh] flex-col items-center justify-center overflow-hidden px-6 pt-24 text-center">
        ...
        <div className="relative z-10 max-w-4xl">
          ...
          <h1 className="mb-8 text-5xl font-black uppercase tracking-tight md:text-8xl">{copy.tagline}</h1>
          <p className="mx-auto mb-10 max-w-2xl text-lg text-white/80 md:text-xl">{copy.description}</p>
      ...
      <section className="container mx-auto grid max-w-6xl gap-6 px-6 py-24 md:grid-cols-3">
        {cards.map(([title, text, category]) => (
          <Link key={title} href={`/${locale}/${title === copy.about ? "about" : title === copy.services ? "services" : "offers"}`} className="rounded-3xl border border-white/10 bg-white/5 p-8 transition hover:bg-white/10">
