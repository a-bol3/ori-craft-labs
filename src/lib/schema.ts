...
export const services = pgTable("services", {
  id: uuid("id").primaryKey(), locale: varchar("locale", { length: 2 }).notNull(), section: varchar("section", { length: 64 }).notNull(), title: text("title").notNull(), subtitle: text("subtitle").notNull(), slug: varchar("slug", { length: 160 }).notNull(), shortDescription: text("short_description").notNull(), valueSummary: text("value_summary").notNull(), priceFrom: text("price_from"), duration: text("duration"), bullets: jsonb("bullets").$type<string[]>().notNull().default([]), isActive: boolean("is_active").notNull().default(true), legacyMongoId: varchar("legacy_mongo_id", { length: 32 }), ...timestamps,
}, (t) => ({ slugUnique: uniqueIndex("services_slug_unique").on(t.slug), localeIndex: index("services_locale_idx").on(t.locale) }));

export const offers = pgTable("offers", {
  id: uuid("id").primaryKey(), group: varchar("group_name", { length: 64 }).notNull(), slug: varchar("slug", { length: 160 }).notNull(), locale: varchar("locale", { length: 2 }).notNull(), title: text("title").notNull(), subtitle: text("subtitle"), priceFrom: text("price_from"), duration: text("duration"), includes: jsonb("includes").$type<string[]>().notNull().default([]), value: text("value").notNull(), idealFor: text("ideal_for"), notes: text("notes"), order: integer("sort_order").notNull().default(0), isActive: boolean("is_active").notNull().default(true), legacyMongoId: varchar("legacy_mongo_id", { length: 32 }), ...timestamps,
...
export const insightPosts = pgTable("insight_posts", {
  id: uuid("id").primaryKey(), title: text("title").notNull(), slug: varchar("slug", { length: 160 }).notNull(), category: varchar("category", { length: 64 }).notNull().default("other"), locale: varchar("locale", { length: 2 }).notNull(), excerpt: text("excerpt").notNull(), content: text("content").notNull(), publishedAt: timestamp("published_at", { withTimezone: true }), legacyMongoId: varchar("legacy_mongo_id", { length: 32 }), ...timestamps,
...
export const legalPages = pgTable("legal_pages", {
  id: uuid("id").primaryKey(), slug: varchar("slug", { length: 64 }).notNull(), locale: varchar("locale", { length: 2 }).notNull(), title: text("title").notNull(), intro: text("intro"), content: text("content").notNull(), lastUpdated: timestamp("last_updated", { withTimezone: true }).notNull().defaultNow(), isActive: boolean("is_active").notNull().default(true), legacyMongoId: varchar("legacy_mongo_id", { length: 32 }), ...timestamps,
...
export const sqlTables = { users, contactRequests, newsletterSubscribers, rateLimitBuckets, heroSettings, services, offers, insightPosts, legalPages, mediaAssets, pages, partners, auditLogs };
